...
int main(int argc, char *argv[]) {
    ...
    #pragma omp parallel
    {
        #pragma omp single
        {
            ...
            thread_cents = calloc(C * nthreads * S, sizeof(double)); /* C cabinets × nthreads × S features */
            thread_cnts = calloc(C * nthreads, sizeof(int)); /* C cabinets × nthreads */
        ...
        while (changed) {
            ...
            for (int d = 0; d < D; d++) {
                ...
                for (int c = 0; c < C; c++) {
                    double dist = 0.0;
                    double *cen_row = &centroids[c*S];

                    #pragma omp reduction(+:dist)
                    for (int s = 0; s < S; s++) {
                        double diff = doc_row[s] - cen_row[s];
                        dist += diff * diff;
                    }
                    if (dist < best_dist) { 
                        best_dist = dist; 
